...
Version 0.2.1 - This version of the code is designed to use cython
                to speed up calculations.
                This version has also been drastically re-written to
                speed up the calculations, as a result, the plotting
                section requires a total overhaul and the previous
                code deleted from this version
"""
...
def possible_coords(mof, cell, multi):
    ...
    for nid, atom in enumerate(mof):
        ...
        for fcoord in fcoords[atom]:
            ...
            if fcoord == fracs:
                ocoords[atom] = ccoord
    return (ccoords, fcoords, ocoords)
...
def calculate_rdf2(mof, atoms, coords, ocoords):
    """Calculates the RDF"""
...
def main():
    ...
    elif os.path.exists(gen_file) and not RERUN:
        f_break = True
        rdfs, rbins, breakdown = load_rdf('%s_None%s' % (nmof, tail))
        print "Previous General Calculation Found."
    else:
        rbins, rdfs, breakdown = calculate_rdf2(mof, atoms, coords[0], coords[2])
